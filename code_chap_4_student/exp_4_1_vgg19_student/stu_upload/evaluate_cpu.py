...
def vgg19():
    ...
    for i, layer_name in enumerate(layers):
        if layer_name.startswith('conv'):
            # TODO: 在时序容器中传入卷积运算
            layer_container.add_module(layer_name, nn.Conv2d(in_channels,cfgs[i],
            ...
            layer_container.add_module(layer_name, nn.MaxPool2d(kernel_size=2))
            # ________________________________________________
            ...
            layer_container.add_module(layer_name, nn.Softmax(1))
            # ________________________________________________
...
def load_image(path):
    ...
    img = img.unsqueeze(0)
    # ________________________________________________
    # ps: why macbook 's code correction so convenient
    #
    return img
